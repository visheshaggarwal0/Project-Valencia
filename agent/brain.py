...
""")
    ...
    def _get_relevant_tools(self, user_input: str) -> List[Any]:
        ...
        if any(kw in user_input for kw in ["notepad", "calc", "file", "time", "screen", "click", "type", "keyboard", "window", "vol", "mute"]):
            categories.append("WINDOWS")
        if any(kw in user_input for kw in ["search", "google", "browser", "open http", "url", "click link", "who is", "what is the"]):
    ...
    def _create_llm(self, model: str = "llama3.1", tools: List[Any] = None):
        ...
        if self.provider == "ollama":
            ...
            model_map = {"gemma3:1b": "gemma2-9b-it", "llama3.1": "llama-3.1-70b-versatile"}
...
Response:"""
        try:
            route: AIMessage = self.router_llm.invoke([SystemMessage(content=router_prompt)])
