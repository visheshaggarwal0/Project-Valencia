...
"""
    ...
    def _apply_rules(self, user_input: str) -> Optional[str]:
        ...
        # 0. Greetings Bypass (Token Optimization)
    ...
    def run(self, user_input: str, max_steps: int = 10, status=None, context: str = "") -> str:
        ...
        # Build ephemeral history with context so RAG blocks don't permanently bloat the session memory
        current_history = list(self.history)
        if context:
            current_history.insert(-1, SystemMessage(content=context))
...
Response:"""
        ...
        # 4. Reasoning Step
        if "CHAT" in intent:
            response: AIMessage = self.router_llm.invoke(current_history)
        ...
        # TOOL Intent -> Planning Step
        planner_prompt = SystemMessage(content="""You are the Planner.
Given the conversation history and the user's latest request, create a step-by-step plan of action.
Focus on identifying what tools need to be called and in what order. 
Respond ONLY with the Plan, starting with 'Plan:'. Do NOT call any tools yourself.""")
        
        # Call planner (no tools bound so it can't execute)
        planner_llm = self._create_llm(model="llama3.1", tools=None)
        try:
            plan_response = planner_llm.invoke(current_history + [planner_prompt])
            ...
            current_history.append(plan_msg)
            panel = Panel(
                Markdown(plan_response.content), 
                title="🧠 [bold magenta]Viora Strategy[/bold magenta]", 
            ...
            if status:
                ...
                viora_console.print(panel)
            self._track_tokens(plan_response)
        except Exception as e:
            viora_console.print(f"[dim red]Planner logic skipped: {e}[/dim red]")
